...
INPUT_FILE = 'results/ratings-2025-11-16-18-28.tsv'
...
def analyze():
    ...
    # Print table of pairs and distances to CSV file
    csv_file = 'results/pair_distances.csv'
    with open(csv_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Color A', 'Color B', 'Human', 'RGB', 'HSV', 'Lab76', 'Lab2k'])
        for pair in distances:
            # Colors in 8-bit notation
            # color_a = pair[0]
            color_a_txt = f"({pair[0][0]}, {pair[0][1]}, {pair[0][2]})"
            # color_b = pair[1]
            color_b_txt = f"({pair[1][0]}, {pair[1][1]}, {pair[1][2]})"
            ...
            writer.writerow([color_a_txt, color_b_txt,
                             f"{human_dist:.4f}",
                             f"{rgbl_dist:.4f}",
                             f"{hsv_dist:.4f}",
                             f"{lab76_dist:.4f}",
                             f"{lab2k_dist:.4f}"])
    # print("Pair | Human | RGBD | RGBL | HSV | Lab76 | Lab2k")
    # for pair in distances:
    #     human_dist = distances[pair]['human']
    #     # rgbd_dist = distances[pair]['rgbd']
    #     rgbl_dist = distances[pair]['rgbl']
    ...
    #     lab76_dist = distances[pair]['lab76']
    #     lab2k_dist = distances[pair]['lab2k']
    #     # print(f"{pair[0]} - {pair[1]} | {human_dist:.3f} | {rgbd_dist:.3f} | {rgbl_dist:.3f} | {hsv_dist:.3f} | {lab76_dist:.3f} | {lab2k_dist:.3f}")
    #     print(f"{pair[0]} - {pair[1]} | {human_dist:.3f} | {rgbl_dist:.3f} | {hsv_dist:.3f} | {lab76_dist:.3f} | {lab2k_dist:.3f}")
    ...
    def show_correlation(x_values, y_values, x_label, y_label):
        ...
        plt.xlabel(x_label)
        plt.ylabel(y_label)
        plt.title(f"Correlation between {x_label} and {y_label} distances")
    ...
    # show_correlation(rgbl_x, rgbl_y, "RGB", "Human perceived")
    # show_correlation(hsv_x, hsv_y, "HSV", "Human perceived")
    # show_correlation(lab76_x, lab76_y, "L*a*b* ΔE76", "Human perceived")
